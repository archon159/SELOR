...
def update_base_model(
    ...
    base_save_path = f'./{save_dir}/base_models'

    prefix = f'base_{base}_dataset_{dataset}'
    cands = sorted([c for c in os.listdir(result_path) if c.startswith(prefix)])
    target = cands[-1]
    best_base_model_path = f'{result_path}/{target}/model_best.pt'
    eval_path = f'{result_path}/{target}/model_eval'

    if prefix not in os.listdir(base_save_path):
        os.system(f'mkdir {base_save_path}/{prefix}')

    base_update_path = f'{base_save_path}/{prefix}'

    os.system(f'cp {best_base_model_path} {base_update_path}/model_best.pt')
    os.system(f'cp {eval_path} {base_update_path}/model_eval')
